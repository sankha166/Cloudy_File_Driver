...
type PageEdit = { rotation: number; brightness: number; contrast: number; grayscale: boolean };
...
export function MobileCaptureModal({
  ...
  const addFiles = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setEdits((current) => [...current, ...incoming.map(() => ({ rotation: 0, brightness: 100, contrast: 100, grayscale: false }))]);
  ...
  const savePages = async () => {
    ...
    try {
      ...
      await onUpload(editedFiles, destination);
      onClose();
    } finally {
      setBusy(false);
    }
        ...
        {!files.length ? (
          <div className="capture-start">
            ...
            <div className="capture-choice-grid">
              <button onClick={() => cameraRef.current?.click()}><Camera size={22} /><strong>Scan document</strong><span>Capture one or more pages</span></button>
              <button onClick={() => galleryRef.current?.click()}><ImagePlus size={22} /><strong>Choose from gallery</strong><span>Select multiple images</span></button>
            </div>
          </div>
        ) : (
          <>
            ...
            <button className="capture-add-page" onClick={() => cameraRef.current?.click()}><ScanLine size={17} /> Add another scanned page</button>
            ...
            <div className="capture-footer"><button className="outline-button" onClick={onClose}>Discard</button><button className="primary-button" onClick={savePages} disabled={busy}>{busy ? <Loader2 size={16} className="spin" /> : <Check size={16} />} Save to Drive</button></div>
          </>
        )}
        <input ref={galleryRef} type="file" accept="image/*" multiple hidden onChange={addFiles} />
        <input ref={cameraRef} type="file" accept="image/*" capture="environment" multiple hidden onChange={addFiles} />
      </div>
    </div>
  );
}
